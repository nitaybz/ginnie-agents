...
- **Voice messages are transcribed locally — opt-in at setup.** When enabled, Slack voice memos and audio attachments are transcribed by whisper.cpp on the host before agents see them; the agent receives the transcript spliced into the message text and never sees the audio file. Install via `bash scripts/install-whisper.sh` (the setup skill prompts for it on Y/n) — clones + builds whisper.cpp into `listener/.whisper/` and downloads the `small` multilingual model (~466MB) from HuggingFace. Requires `ffmpeg` on PATH and a C++ toolchain (`cmake`/`make`, `g++`). When not installed, audio attachments fall back to standard download stubs and the listener logs a one-time warning. Zero per-transcription cost, fully offline.
...
- **Interactive buttons have a managed lifecycle.** On a button click the listener strips only the clicked `actions` block (sibling blocks — e.g. a second proposal in the same message — stay live). A periodic sweep (`listener/src/button-sweep.ts`) auto-retires any of an agent's own buttons left untouched past `button_ttl_hours` (per-agent in `config.json`, default 48h), scanning the agent's primary channel. Agents should also `chat.update` their own messages to drop buttons the moment the underlying issue is resolved/withdrawn — expiry is only the backstop.
