...
- `setup-watcher` — wire up the optional Watcher daemon (framework health monitoring with interactive Slack buttons; no AI)
...
The **Watcher** is the canonical bot — a small Node daemon (Bolt + Socket Mode, no AI) that monitors framework health and DMs the operator with interactive buttons (`Update now`, `Restart listener`, `Ack 24h`, `View logs`) plus a `/watcher` slash command. Don't wrap shell scripts in an AI agent; don't wrap reasoning in a script.
