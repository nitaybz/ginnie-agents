...
| **Data** | persistent runtime state (cursors, CRM, scratch) | `agents/<n>/data/` | per-agent (optional) |
...
│   ├── playbook.md
│   └── episodes/YYYY-Qn.md
├── data/                           (rw, optional — agent's persistent runtime state)
│   └── *                           (cursors, CRM, scratch JSON — schema is the agent's, not the framework's)
├── skills/                         (ro, per-agent)
├── .shared/                        (ro)
...
The agent itself only references the unprefixed paths (`./memory/`, `./data/`, `./skills/`, `./schedules.json`). Anything under `.shared/` or `.framework/` is auto-loaded by the entrypoint and shouldn't be read directly from `PROMPT.md`.

The `data/` directory is the framework's generic primitive for per-agent persistent runtime state. The framework mounts `agents/<name>/data/` (if it exists) RW at `/workspace/data` and imposes no schema. Use it for cursors, CRMs, scratch JSON, draft caches — anything that needs to survive across container restarts. Memory (`memory/`) is the parallel primitive for narrative reasoning state (rules, playbook, episodes); `data/` is for machine state.
...
- **Dispatch from unverified Slack senders to write-capable agents.** The listener resolves every inbound sender against `shared/known-users.json` ∪ per-agent `known-users.json` and refuses, at dispatch time, to wake an agent with `boundaries: "write"` for a sender whose role is `unknown` (lookup failed) or `external` (not curated). Defense-in-depth on top of `PROMPT.md` guidance — a successful injection from a curated sender still lands in the agent's container, but a stranger who joins a channel can't reach a write-capable agent at all. Read-only agents are not gated (their blast radius is bounded). Per-agent opt-out: `allow_unverified_senders: true` in `config.json`.
- **Unsigned framework updates** (when opted in). With `FRAMEWORK_REQUIRE_SIGNED_TAG=true`, `scripts/update-framework.sh` requires the upstream tip to be a git tag signed by a key in the operator's `gpg` keyring; otherwise the update is refused before any pull. Default off for backward compat — operators who track `main` retain the prior trust-the-remote model.
